         ...
         while ( (pr_default.getStatus(0) != 101) )
         {
            ...
            while ( (pr_default.getStatus(1) != 101) )
            {
               ...
               else if ( StringUtil.StrCmp(AV10PageItem.gxTpr_Pagetype, "Information") == 0 )
               {
                  ...
                  new prc_logtofile(context ).execute(  context.GetMessage( "Page Name", "")+" : "+A517PageName) ;
                  new prc_logtofile(context ).execute(  context.GetMessage( "Page Info Data", "")+" : "+A518PageStructure) ;
                  new prc_logtofile(context ).execute(  context.GetMessage( "Page Info Data", "")+" : "+AV16SDT_InfoContent.ToJSonString(false, true)) ;
               }
               else
               {
                  AV13SDT_ContentPage.FromJSonString(A518PageStructure, null);
